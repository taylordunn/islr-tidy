...
x <- matrix(rnorm(100 * 100, mean = 0, sd = 1), ncol = 100)
...
```{r}
tidy(m_mult) %>%
  transmute(coef = paste0("beta_", 0:99), estimate_mult = estimate) %>%
  left_join(
    beta_hat_estimates %>%
      filter(!is.na(estimate)),
    by = "coef"
...
Can't get the multiple regression coefficients to converge.
