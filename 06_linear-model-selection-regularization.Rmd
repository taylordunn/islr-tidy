...
Here, the four-variable model was selected, but I would chalk that up to the random sampling.
...
The shapes of the curves are right, but the scale of the $\lambda$ parameter in the left panel is much different -- not sure what happened there.
...
#### Bayesian Interpretation for Ridge Regression and the Lasso {0}
...
The model is then re-fit using all available observations that the selected tuning parameter $\lambda$.
...
```{r}
penalty_grid <- grid_regular(penalty(range = c(-4, 0)), levels = 10)
penalty_grid
```
