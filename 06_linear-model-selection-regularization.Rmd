...
(c) Fit a lasso regression model, with $\lambda$ chosen by cross-validation.
...
college_lasso_last_fit %>%
  str(max.level = 2)
college_lasso_last_fit$.predictions
collect_predictions(college_lasso_last_fit)
```
...
bind_rows(
  ...
  mutate(model = factor(model, c("lm", "ridge", "lasso"))) %>%
