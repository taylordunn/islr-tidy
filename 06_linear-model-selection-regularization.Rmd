...
Lasso also selects the 7th order coefficient, but the estimates are much different, I think due to `poly()`?
...
#### 10. Training/testing errror vs features on simulated data {-}
...
It is a very heavy tailed distribution.
...
(autoplot(boston_pca_tune) + labs(title = "PCA")) |
  (autoplot(boston_pls_tune) + labs(title = "PLS"))
```
...
```{r}
tidy(boston_lasso_last_fit$.workflow[[1]])
```
