...
We have seen in the example that when $p > n$, it is easy to obtain a useful model that has zero residuals.
  ...
  geom_vline(
    data = . %>%
      ...
      filter(value == ifelse(str_detect(metric, "r.squared"),
                             max(value), min(value))),
    aes(xintercept = n_vars), size = 1
  ) +
  facet_wrap(~ metric, scales = "free_y")
```

As expected, $R^2$ increases monotonically.
The best number of variables by adjusted $R^2$, $C_p$ and BIC are 11, 6 and 10, respectively.
...
And the results are also output in a vary particular way.
Here are the coefficients associated with the 50th $\lambda$ value (= `r ridge_salary$lambda[50]`) and the 60th (= `r ridge_salary$lambda[60]`):

```{r}
coef(ridge_salary)[, 50]; coef(ridge_salary)[, 60]
```

The coefficients in the first model are much smaller compared to the second, as expected.
...
The warnings here are coming from the `rsq` metric blowing up at large penalty values, which I can quickly visualize with `autoplot()`:
...
# Note that `augment()` works here because the fit is finalized, with a single
#  `penalty` value -- previously I had to use `bind_cols(predict(...))`
...
Knowing this, I'm going to constrain `penalty_grid` to a smaller range.
...
The advantage is that the lasso has performed variable selection for us bty setting some to exactly zero:
