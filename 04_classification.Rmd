...
There is a function called `make_blobs()` in the scikit-learn package ([documentation here](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.make_blobs.html) that I've used before to simulate clustered data.
From a quick Google search, I found [this R package `clusteringdatasets`](https://github.com/elbamos/clusteringdatasets/) which translates the function in R.
I adapted the [source code](https://github.com/elbamos/clusteringdatasets/blob/master/R/sklearn.R) to simulate the two-class linear scenarios:
...
Simulate 100 testing data sets for each of these scenarios:

```{r sim-linear-train, cache=TRUE}
...
The text doesn't specify how many training samples are tested, just that it is a "large test set."
I'll use 1000:

```{r sim-linear-test, cache=TRUE}
...
Now load `tidymodels` and define the different models to be evaluated:
...
```{r sim-linear-res, cache=TRUE, warning=FALSE, message=FALSE}
tic()
...
Plot error rate for each model and scenario as in Figure 4.11:
...
* Scenario 1: uncorrelated normal variables.
    ...
    * Naive Bayes should be better than QDA because of independent predictors (as in the text) but that is not the case in my simulations.
...
I'm not sure of an easy way to adapt this approach to the non-linear scenarios 4, 5 and 6.
If anyone has ideas, please share them [on this issue](https://github.com/taylordunn/islr-tidy/issues/1).
Here is a summary of the results presented in Figure 4.12:
...
>Finally, recall from Chapter 3 that in the regression setting we can accommodate a non-linear relationship between the predictors and
...
knn_spec5 <-
  nearest_neighbor(mode = "classification", engine = "kknn", neighbors = 5)
```
