...
QDA gets its name from how the quantity $x$ appears as quadratic function in the first term of the above equation.
...
In short: use LDA if there are relatively few training observations, and use QDA for many.
...
But naive Bayes can still perform well, especially when $n$ is not large enouugh relative to $p$ to effectively estimate the joint distribution of the predictors within each class.
...
The is the familiar log-odds of class $k$ compared to baseline class $K$.
...
```{r}
qda_direction_fit <-
  discrim::discrim_quad(mode = "classification", engine = "MASS") %>%
  fit(Direction ~ Lag1 + Lag2, data = smarket_train)
qda_direction_fit
```
...
```{r}
nb_direction_fit <-
  discrim::naive_Bayes(mode = "classification", engine = "klaR") %>%
