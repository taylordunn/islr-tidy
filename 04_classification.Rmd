...
make_blobs <- function(
  ...
  if (is.matrix(centers)) {
    ...
    centers <- runif(n  =  n_features * centers,
                     min = center_box[1], max = center_box[2])
    centers <- matrix(centers, ncol = n_features)
  }
  
  if (length(cluster_std) != 1 & length(cluster_std) != nrow(centers)) {
    stop("cluster_std must be 1 or the same length as the number of clusters")
  }
  ...
  if (length(cluster_std) == 1) {
    points <- starting_points * cluster_std
  } else {
    points <- starting_points * cluster_std[categories]
  }
  
  points <- points + centers[categories, ]
  
  list(
    samples = points,  labels = categories
  )
}

blobs <- make_blobs(centers = matrix(c(1, -1, 1, -1), ncol = 2))
d <- as_tibble(blobs$samples) %>%
  setNames(c("x1", "x2")) %>%
  bind_cols(y = factor(blobs$labels))
d %>%
...
x <- MASS::mvrnorm(n = n_obs, mu = c(mu_1 , mu_2), Sigma = sigma)
colnames(x) <- c("x1", "x2")
x <- as_tibble(x)
...
beta_1 <- 1
beta_2 <- 1
log_odds <- beta_0 + beta_1 * x$x1 + beta_2 * x$x2
y_prob <- 1 / (1 + exp(-log_odds))
