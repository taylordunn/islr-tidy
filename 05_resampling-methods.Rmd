...
* $k$-fold CV (average over $k$ fitted models) tends to have lower variance than LOOCV (average over $n$ fitted models, highly correlated with eachother).
...
d4 <- tibble(sim = 1:4) %>%
  ...
  ungroup() %>%
  mutate(
    alpha = map2_dbl(x, y, sim_alpha) %>% round(3)
  ) %>%
  unnest(cols = c(x, y))
d4 %>%
  ggplot(aes(x, y)) +
