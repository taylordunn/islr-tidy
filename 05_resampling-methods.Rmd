...
```{r}
validation_set_split <- function(seed) {
  set.seed(seed)
  auto_split <- initial_split(auto, prop = 0.5)
  list(train = training(auto_split), validation = testing(auto_split))
}

evaluate_poly_fit <- function(auto_train, auto_validation) {
...
auto_splits <- validation_set_split(seed = 300)
evaluate_poly_fit(auto_splits$train, auto_splits$validation)
```
...
d <- map_dfr(
  1:10,
  function(seed) {
    auto_splits <- validation_set_split(seed)
    evaluate_poly_fit(auto_splits$train, auto_splits$validation)
  },
  .id = "rep"
)
