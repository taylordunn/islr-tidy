...
sigma <- matrix(c(sigma_x, sigma_xy, sigma_xy, sigma_y), nrow = 2)
...
I don't have access to the simulation parameters, so I'll just use a standard normal distribution (mean 0, SD 1) for both variables:

```{r}
sim_alpha <- function(x = NULL, y = NULL) {
  # Allow the function to provide the x and y values, or simulate if missing
  if (is.null(x)) x <- rnorm(100)
  if (is.null(y)) y <- rnorm(100)
  (var(y) -  cov(x, y)) / (var(x) + var(y) - 2 * cov(x, y))
}
sim_alpha()
```

Since this is a simulation, we can simply generate new data and get many estimates $\hat{\alpha}$:

```{r figure5.9}
d4 <- tibble(sim = 1:4) %>%
  rowwise() %>%
  mutate(x = list(rnorm(100)), y = list(rnorm(100))) %>%
  ungroup() %>%
  mutate(alpha = map2_dbl(x, y, sim_alpha) %>% round(3)) %>%
  unnest(cols = c(x, y))
d4 %>%
  ggplot(aes(x, y)) +
...
d <- bind_rows(
  d %>% mutate(estimates = "true"),
  tibble(alpha = d1_boot_alpha, estimates = "bootstrap")
) %>%
  mutate(estimates = fct_rev(estimates))

p1 <- d %>%
  ggplot(aes(x = alpha, fill = estimates)) +
  geom_histogram(binwidth = 0.025, show.legend = FALSE) +
  facet_wrap(~ estimates, nrow = 1) +
  scale_fill_manual(values = c(td_colors$nice$soft_orange,
                               td_colors$nice$strong_blue))
p2 <- d %>%
  ggplot(aes(y = alpha, x = estimates)) +
  geom_boxplot(aes(fill = estimates), show.legend = FALSE) +
  scale_fill_manual(values = c(td_colors$nice$soft_orange,
                               td_colors$nice$strong_blue))
p1 | p2
```
...
```{r}
auto_poly_fits_loo_cv %>% mutate(mse = rmse^2)
```
