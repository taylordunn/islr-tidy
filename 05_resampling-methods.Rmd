...
The `tidymodels` approach to bootstrap estimates, as in \@ref(bootstrap):
...
#### Estimating the Accuracy of a Regression Model {-} {#bootstrap-regression}
...
Instead of the workflow I used in \@ref(bootstrap-regression) (with `fit_resamples()`), I'll show a different way.
...
```{r}
default_boot <- bootstraps(default, times = 1000)
